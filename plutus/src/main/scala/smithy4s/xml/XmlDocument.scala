...
object XmlDocument {
  ...
  /** The XmlContent is the the very simple ADT that Smithy4s works against when
    * dealing with XML. It can be either a piece of text, or an XML element.
    * Smithy4s expects XML references to be resolved before this content is
    * created.
    *
    * It is worth noting that comments and other miscellaneous elements are
    * erased before instances of this ADT are produced
    */
  // format: off
  ...
  /** A Decoder aims at decoding documents. As such, it is not meant to be a
    * compositional construct, because documents cannot be nested under other
    * documents. This aims at decoding top-level XML payloads.
    */
  trait Decoder[A]:
    ...
    def fromSchema[A](schema: Schema[A]): Decoder[A] =
      ...
      new Decoder[A]:
        def decode(xmlDocument: XmlDocument): Either[XmlDecodeError, A] =
          ...
          if rootName != expectedRootName then
            Left(
              XmlDecodeError(
                XPath.root,
          ...
          else decoder.decode(XmlCursor.fromDocument(xmlDocument))
  ...
  object Encoder:
    def fromSchema[A](schema: Schema[A]): Encoder[A] =
      ...
      if xmlEncoder.encodesUnion then
  ...
  /** This instance implements the DocumentBuilder interface provided by
    * fs2-data, which can be used to parse a stream of XML events into a stream
    * of our XmlDocument.
    */
  implicit val documentBuilder: DocumentBuilder[XmlDocument] =
    new DocumentBuilder[XmlDocument]:
      ...
      def makeText(texty: XmlEvent.XmlTexty): Content = texty match
        ...
        case XmlString(s, _) =>
          if s.trim.isEmpty then None else Some(XmlDocument.XmlText(s))
