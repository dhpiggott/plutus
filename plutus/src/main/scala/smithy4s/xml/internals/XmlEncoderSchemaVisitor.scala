...
private[smithy4s] abstract class XmlEncoderSchemaVisitor
      ...
      val encoder = field
        .foldK(new Field.FolderK[Schema, S, XmlEncoder]:
          def onRequired[AA](
            ...
            if isAttribute then compile(instance).attribute(xmlName)
          ...
          def onOptional[AA](
            ...
            if isAttribute then compile(instance).attribute(xmlName).optional
  ...
  def collection[C[_], A](
    ...
    new XmlEncoder[C[A]]:
      ...
      val memberWriter =
        if isFlattened then compile(member) else compile(member).down(xmlName)
  ...
  def enumeration[E](
      ...
      total: E => EnumValue[E]
  ): XmlEncoder[E] = if hints.has(IntEnum) then
    new XmlEncoder[E]:
  ...
  def union[U](
    ...
    val underlying = dispatch.compile(
      new Alt.Precompiler[Schema, XmlEncoder]:
