...
/** This constructs allow for encoding XML data. It is not limited to top-level
  * documents, and works by returning a list of XmlContent, that can be nested
  * under other elements.
  *
  * It exposes simple combinators that allow to indicate that the encoding
  * should happen further down under an element, or that the decoding should
  * return nothing when dealing with the absence of data (which maybe be the
  * case for optional data)
  */
private[smithy4s] trait XmlEncoder[-A]:
  self =>
