...
/** This constructs allow for decoding XML data. It is not limited to top-level
  * documents, and works against XmlCursor. Smithy4s does not have vocation to
  * be a general-purpose XML library, so we keep this as a private
  * implementation detail that should never be used directly.
  *
  * It exposes simple combinators that allow to indicate that the decoding
  * should happen further down the cursor, or enable the prevention of failure
  * when decoding an absence of node (which is the case for optional data).
  */
private[smithy4s] trait XmlDecoder[A]:
  self =>
...
private[smithy4s] object XmlDecoder:
  ...
  /** This is the method that is used to define primitive decoders, as all
    * primitives are decoded from text content, whether it's in elements or in
    * attributes.
    */
