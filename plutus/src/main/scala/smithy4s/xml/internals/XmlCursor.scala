...
/** This construct is an internal implementation-detail used for decoding XML
  * payloads.
  *
  * It echoes the model popularised by Argonaut and Circe, where "cursors" are
  * used instead of the direct data. This makes it easier to express the
  * decoding logic that needs to "peek further down" the XML data.
  */
...
private[smithy4s] object XmlCursor:
  ...
  case class Nodes(
    ...
    def down(tag: XmlQName): XmlCursor = if isSingle then
    ...
    def attr(name: XmlQName): XmlCursor = if isSingle then
      val node = nodes.head
