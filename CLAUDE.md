...
- **`claude-code-review.yml` runs on Linux and cannot build anything.** It is a read-only reviewer; treat "does this compile?" as CI's answer, not something that review can establish. Two of its inputs are load-bearing: `allowed_bots: "claude"`, because the action refuses to run for a bot actor and Claude's own pushes would otherwise fail a required check; and `track_progress: true`, because supplying a `prompt` puts the action in automation mode, where findings go to the run log instead of the pull request.
- **Checkout is `fetch-depth: 0` everywhere on purpose**, for two different reasons. In the workflows that build, sbt-dynver derives `version` from the tag history and that version lands in `BuildInfo`, so a shallow clone silently yields `0.0.0+…`. In `claude-code-review.yml`, which never builds, it is the diff and history the reviewer reads.
...
`main` is governed by a repository ruleset: pull requests required with no bypass actors, `Build` and `Claude review` must pass, commits must be signed, and only squash and merge commits are allowed.
...
- **A job's `name` is its check-run name, and two of those are ruleset contexts.** Renaming a job therefore breaks merging until the ruleset's required contexts are updated — including for the renaming PR itself, which reports under the new names while the old ones are still required. Sequence: open the PR, let its checks report, update the contexts, then merge.
