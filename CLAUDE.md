...
For a heavier pass, `claude.yml` also installs the `code-review@claude-code-plugins` skill, invoked by name:

```
@claude /code-review:code-review --comment dhpiggott/plutus/pull/<N>
```

It fans out to four parallel reviewers — two on CLAUDE.md compliance, two on bugs — then validates every finding with a second agent before posting. `--comment` is not optional: its step 7 stops without posting when the flag is absent. It needs `Skill` and `Task`, which the action does not grant, so it works only because `claude.yml` runs under `--permission-mode auto`.
