...
- **Newspaper ordering (Clean Code ch. 5)**: a definition goes *above* the ones it calls, and its callees follow in order of first use, so a file reads top-down from the command entry point to the smallest helper. `GnuCashCommands.scala` is the worked example — the three `*Opts`/command pairs, then `withBook`, then `promoteAbandonedBackups`, `bookDirectory`, `promoteBackup`, then the formatters. `requireExistingBook` is the one exception, sitting above `withBook` and `importTransactions` rather than below both. Nothing enforces this, so it is the easiest convention to break when adding a helper: put a new one below its caller, not next to the code that gave you the idea for it.
