...
- **Newspaper ordering (Clean Code ch. 5)**: a function goes *above* the ones it calls, and a helper with more than one caller sits below the first of them, so a file reads top-down from the command entry point to the smallest helper. `GnuCashCommands.scala` is the worked example — the three `*Opts`/command pairs, then `withBook` and `requireExistingBook`, then `promoteAbandonedBackups`, `bookDirectory`, `promoteBackup`, then the formatters. The only definitions above their first use are the two `DateTimeFormatter` constants, each paired with the single function that formats with it (`backupTimestamp`/`formatBackupTimestamp`, `consoleTimestamp`/`formatConsoleTimestamp`); a constant belongs with its function rather than below it. The rule holds across the rest of `main` too, and what remains above its first use anywhere is that same pairing (`gnuCashTimestamp`/`formatTimestamp`, `neutralTime`/`neutralPostDate`, `SlotTypeString`/`stringSlot`, `OnlineId`/`onlineIds`, `monzoApiUri`/`withMonzoApi`, `callbackPort`+`redirectUri`/`exchangeAuthCode`, `monzoAuthUri`/`requestAuthorization`, `ofxDateTimeFormatter` and `gbpMinorUnitsPerMajorUnit`/`majorUnits`) — plus companion objects and the `Sqlite` statement fields, which are declaration placement rather than call order and which the rule doesn't speak to. Two files are deliberately outside it: `porcupine/` is an inlined upstream fork, kept diffable against it, and `TokenExchangeCodecs.scala` follows the shape of smithy4s's own codec objects (down to the upstream comment about `X-Amzn-Errortype`) — and its `val` chain is initialisation-order-sensitive, so `jsonCodecs` and the encoders derived from it can't be reordered freely anyway. Nothing enforces any of this, so it is the easiest convention to break when adding a helper: put a new one below its caller, not next to the code that gave you the idea for it.
