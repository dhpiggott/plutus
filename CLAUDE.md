...
- **A green Claude check is not evidence Claude did anything.** The action runs headless with `--permission-mode acceptEdits`, and its own source notes that with no prompt handler anything falling through to "ask" is denied — so a missing tool is denied silently and the job still exits 0. The tells are in the run log's result block: `permission_denials_count` above zero, and `No buffered inline comments` from the post-step.
