...
## Per-platform sources inside `main`

Not every platform difference needs a module pair. `sbt-projectmatrix` already puts `main/src/main/scalajvm` on `main3` and `main/src/main/scalanative` on `mainNative3` (plus the `-3` and `java*` variants), with no `build.sbt` change, and `scalafmtCheckAll` / `scalafixAll` cover them. `object Host` — `pid` for the `gnclock` row, `ProcessHandle.current.pid` on one row and `unistd.getpid()` on the other — is the whole of it today. Reach for a `keychain`/`porcupine`-style module pair only when there's FFI codegen to configure.

**A javalib gap fails at `nativeLink`, not at `compile`.** `ProcessHandle.current.pid` compiles happily on the Native row and only then reports `Unknown static method`, and Native links only what's *reachable* from `main`, so an unreferenced helper proves nothing. `verify.sh` stops at `compile` and `ci.yml` gates `nativeLink` on `push`, so a PR using a missing JDK class goes green through review and breaks on the post-merge build. Anything touching `java.*` on both rows wants `sbt 'main3/compile' 'mainNative3/nativeLink'`.
...
Where a function *about* a row type lives follows from the same split, and `Account` is where it actually bites, because most of the code that manipulates accounts isn't SQL. A row type owns what is answerable from its own rows — the queries, plus any composition of them that needs no notion of where a row *ought* to be. Everything that decides placement lives in `GnuCashCommands.scala`: the Archive subroot and the mirror chains either side of it, the canonical path a Monzo-backed asset account belongs at, and anything a run prints or would have written. Two mechanical tells keep the line visible, and both are worth checking after a change here: **no function in `Account.scala` takes `dryRun` or `Verbosity`**, and **`GnuCashCommands.scala` contains no `sql"""`**. A helper that wants both sides is in the wrong shape — split it, rather than picking a file for it.

That is why `newGuid` sits in its own `Guid.scala` (accounts, transactions and splits are all identified that way, so it belongs to no one table), why writing an account's `online_id` slot is `Account.tagOnlineId` while the fact that the value is a Monzo account ID stops at the call site, and why the frontier of hidden accounts `archive-accounts` moves (`hiddenAccountsToArchive`) is in the command file despite touching only the `accounts` table: it knows about the Archive subroot.

`BookRoots` — the root account, plus the Archive subroot as a memoised `IO` — is threaded through the placement functions as an ordinary parameter. The subroot has to stay an `IO`: the first retired account is what creates it, so resolving it eagerly would bring one into being in a run that turns out to have nothing to retire. Memoising it is what keeps a run to one creation, one root query, and (in a dry run) one fabricated guid and one "would create" line, however many accounts ask.
...
`GncLock` is GnuCash's own cooperative lock (`gnclock`), taken inside `withBook` as a `Resource` so it covers `restore-account`'s prompt as well as the writes. Three things about it are load-bearing and easy to undo: a dry run *reads* the lock but never inserts one (inserting would take the write lock `withoutCommitting`'s deferred `begin` exists to avoid, and would trip `withBook`'s "a dry run changed nothing" assertion); `withBook` counts rows changed from a baseline taken *after* the lock, because our own `gnclock` row is a row change and would otherwise keep a backup of an unchanged book; and refusal raises `BookInUse`, which `withBook`'s `onError` deletes the `.bak.tmp` for rather than promoting it, since a run that was never allowed to start is not a run that failed. `gnclock` is also the only table the code probes `sqlite_master` for — a book can genuinely not have it, and that case warns rather than failing.
...
- **Newspaper ordering (Clean Code ch. 5)**: a function goes *above* the ones it calls, and a helper with more than one caller sits below the first of them, so a file reads top-down from the command entry point to the smallest helper. `GnuCashCommands.scala` is the worked example — the three `*Opts`/command pairs, then `withBook` and `requireExistingBook`, then `promoteAbandonedBackups`, `bookDirectory`, `promoteBackup`, then the formatters. The only definitions above their first use are the two `DateTimeFormatter` constants, each paired with the single function that formats with it (`backupTimestamp`/`formatBackupTimestamp`, `consoleTimestamp`/`formatConsoleTimestamp`); a constant belongs with its function rather than below it. Nothing enforces any of this, so it is the easiest convention to break when adding a helper: put a new one below its caller, not next to the code that gave you the idea for it.
- **A row type leads its file; its companion object follows it.** Seven types pair a class with a companion — `Account`, `Slot`, `Commodity`, `Split`, `Posting`, `AssetAccounts` and `BookRoots` — and the order used to be split three against four. Class first, because that is what the rule above already implies wherever the two actually depend on each other: the class body reads `Account.decoder` and `Account.selectAccountsWithFlags` (eight references between them) and `Split.NotReconciled`/`ReconcileEpoch`, while no companion reads a class member back. Everywhere else the only edge is a factory calling the constructor — `Slot(…)`, `Posting(…)`, `AssetAccounts(…)` — which the rule doesn't speak to, since a case class's constructor is generated from its field list rather than written as a definition to read. Two things fall out that are worth having anyway: the `/** sqlite> .schema … */` block documenting the table opens the file instead of sitting 90 lines down, and a file named for a type says what that type is before saying what you can do with one. The cost is that the companion's factories — `Account.root`, `Commodity.gbp`, `Posting.fromMonzo`, `AssetAccounts.default` — are how *other* files reach these types and now sit at the foot; that is accepted because the rule is about reading one file top-down, and a caller elsewhere arrives with a qualified name that already says where to look. Nothing here is a compile constraint: companion objects initialise lazily, so either order links.
