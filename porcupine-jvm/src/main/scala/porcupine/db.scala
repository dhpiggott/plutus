...
abstract class Database[F[_]] private[porcupine]:
  ...
  final def cursor[A, B](
      query: Query[A, B],
      args: A
  ): Resource[F, Cursor[F, B]] =
...
private abstract class AbstractDatabase[F[_]](using F: MonadCancelThrow[F])
    extends Database[F]:

  final def execute[A, B](query: Query[A, B], args: A) =
    prepare(query).use(_.execute(args))

  @targetName("executeVoid")
  final def execute[A](query: Query[A, Unit], args: A) =
    prepare(query).use(_.execute(args))

  final def option[A, B](query: Query[A, B], args: A) =
    prepare(query).use(_.option(args))

  final def unique[A, B](query: Query[A, B], args: A) =
    prepare(query).use(_.unique(args))
...
private abstract class AbstractStatement[F[_], A, B](using
    F: MonadCancelThrow[F]
) extends Statement[F, A, B]:

  final def execute(args: A) = cursor(args).use(_.fetch(Int.MaxValue).map(_._1))

  final def execute(using Unit <:< B)(args: A) =
    cursor(args).use(_.fetch(1).void)
