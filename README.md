...
- Before any non-dry run the book is copied to `<input>.bak`, replacing the copy the previous run left — the backup is of the run about to happen, so it undoes that run and no earlier one. The whole write runs in a single SQLite transaction, so a mid-run failure rolls back to the pre-run state (and the backup is the belt-and-braces restore).
