...
A single fetch spans your current account, joint account and pots, and each Monzo account maps deterministically to a GnuCash asset account by its **type** via `AssetAccounts.default` (e.g. `uk_retail → Assets:Current Assets:Monzo:Current`) — no per-account flags. Transactions from an account type not in the map are skipped with a warning. Pot backing accounts carry no type or name over the API, but pot-transfer metadata links each to its pot and [List Pots](https://docs.monzo.com/#list-pots) supplies the pot's name, so each pot posts into its own child of the `Pots` account (e.g. `…:Monzo:Pots:Savings`), created on first sight. That linkage needs a pot transfer somewhere in the fetched window, though: if a pot's only activity in the window is interest (possible for an already-bookmarked pot), nothing identifies which pot the backing account belongs to, so its transactions post into `Pots` itself and a warning names the unlinked account — widen `--since` past a transfer to file it properly. Each transaction becomes one balanced GnuCash transaction with two splits: the signed amount on that account's asset account, and its negation on a category account.

Monzo's own transaction `category` is authoritative for filing: each category becomes a child of `Expenses` named by title-casing it (`eating_out → Expenses:Eating Out`), created on first sight — there is no mapping to maintain. A transaction with no category files under `Expenses:General` (Monzo's default category). Created accounts inherit their parent's account type and commodity; everything else — the mapped asset accounts, the `Pots` parent, and `Expenses` itself — must already exist, and a missing one fails the run before anything is written.
