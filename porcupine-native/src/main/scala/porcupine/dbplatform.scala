...
private abstract class DatabasePlatform:
        ...
        .make {
          F.blocking {
            ...
            try guard(sqlite3_open_v2(fn.at(0), db, flags, null))
              ...
              case t: Throwable =>
                if !db ne null then guard(sqlite3_close(!db))
                throw t
  ...
  inline private def guard(thunk: => CInt): Unit =
    ...
    if rtn != SQLITE_OK then
      throw new RuntimeException(fromCString(sqlite3_errstr(rtn)))

  inline private def guard(db: Ptr[sqlite3])(thunk: => CInt): Unit =
    if thunk != SQLITE_OK then
      throw new RuntimeException(fromCString(sqlite3_errmsg(db)))
