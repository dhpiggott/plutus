...
lazy val readGnucashOpts: Opts[IO[Unit]] = Opts.subcommand(
  name = "read-gnucash",
...
def readGnucash(input: fs2.io.file.Path): IO[Unit] =
  Database
    .open[IO]:
      input.toString
    .use: db =>
      ...
      for
        rootAccount <- Account.root
        hidden <- rootAccount.hiddenChildren
        _ <- archive:
          hidden
      yield ()

// TODO: Add unarchive too?
...
def archive(accounts: List[Account])(using db: Database[IO]): IO[Unit] =
  for
    ...
    _ <- (IO.traverse:
      ...
      for
        path <- account.path
        archiveParent <- account.createOrRetrieveArchiveParent(root)
        // TODO: Remove
        archiveParentPath <- archiveParent.path
        _ <- IO.println:
          s"${path.map(_.name).mkString("/")} archive parent:\n  ${archiveParentPath
              .map(_.name)
              .mkString("/")}"
        // TODO: Test this is working correctly with before/after comparisons.
        _ <- account.moveTo(archiveParent)
      yield ()
  yield ()
...
final case class Account(
  ...
  def hiddenChildren(using db: Database[IO]): IO[List[Account]] = for
    directChildren <- directChildren
      ...
      for
        ...
        archiveParent <- maybeParent match
          ...
          case Some(parent) if parent == root =>
            root.createOrRetrieveArchiveParent(
              parent = root,
              name = "Archive"
            )
  ...
  def moveTo(parent: Account)(using db: Database[IO]): IO[Unit] =
    db.execute(
      // TODO: Set to unhidden too?
      query = sql"""
        update accounts
